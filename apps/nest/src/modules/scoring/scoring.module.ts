...
import type { ScoringDispatchProducer } from "../queues/producers/scoring-dispatch.producer";
...
import { ScoringPersistenceService } from "./services/scoring-persistence.service";
...
export class ScoringDispatchListener {
	 ...
	 */
	constructor(
		private readonly scoringDispatchProducer: ScoringDispatchProducer,
	) {}
	...
	async handle(event: NewProcurementNoticeEvent): Promise<void> {
		if (!event.procurementNoticeId || !event.secopId) {
			this.logger.warn(
				"Skipping scoring dispatch for invalid procurement notice event",
			);
			return;
		}
