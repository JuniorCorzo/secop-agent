...
| Scheduler | Hermes (external Go agent) |
...
│   ├── queues/                      # ✅ Implemented (BullMQ producers/workers)
...
### 4.2 Procurement Notices Module (`modules/procurement-notices`)
...
- `id` (UUID PK), `secopId` (unique), `title`, `description`, `status`
- `entityName`, `contactInfo`, `value`, `currency`
- `publicationDate`, `deadlineDate`, `sector`, `location`
- `sourceMetadata` (JSONB), `createdAt`, `updatedAt`, `deletedAt` (soft delete)
...
### 4.3 Queues Module (`modules/queues`)

**Purpose**: Async job processing via BullMQ for background tasks.
...
│   └── procurement-ingestion.producer.ts   # Enqueues batch ingest jobs
└── workers/
    ├── example-queue.worker.ts             # Template for new workers
    └── procurement-ingestion.worker.ts     # Processes ingestion jobs
```

**Queue Names**: `example-queue`, `procurement-notice-ingestion`

**Pattern**:
- `ProcurementIngestionProducer.enqueueBulkIngestion(dto)` → adds job to queue
- `ProcurementIngestionWorker` processes job → calls `ProcurementNoticesService.bulkUpsert()`

**Config**: Redis connection via `ConfigService` (host, port, password from env).

### 4.4 Health Module (`modules/health`)
...
app.useGlobalPipes(new ValidationPipe({              // class-validator on all routes
