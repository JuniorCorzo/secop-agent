# soda-scheduler Specification

## Purpose

Provide a periodic SODA ingestion scheduler that runs inside NestJS, fetching SECOP-I and SECOP-II datasets directly from datos.gov.co via SODA 3.0 API and persisting normalized records to PostgreSQL without depending on external agents.

## Non-Goals

- This spec does not cover AI-based normalization or enrichment of ingested records.
- Hermes or any external scheduler agent is not part of this ingestion path.
- Historical backfill beyond the incremental window is out of scope.

## Constraints

- Scheduler MUST use `@nestjs/schedule` with a configurable cron expression.
- Datasets MUST be fetched in parallel using `Promise.allSettled` so one failure does not block the other.
- Page size MUST default to 5000 to avoid SODA request timeouts.
- Incremental fetching MUST use `lastRunTimestamp` stored in memory per dataset.
