...
## 5. Scoring, Batch Processing & LLM Integration (PR 19)

A partir de las adiciones recientes (Sprint 3), el sistema incluye el soporte de **Batch Scoring Asíncrono**, auditoría con **Score Logs**, y una capa abstracta de integración con **LLM**.

### 5.1 Flujo de Batch Scoring & LLM Provider

El procesamiento de scoring soporta lotes (batch) enviados a la cola de BullMQ. Dentro del pipeline de Scoring, se invocan de manera abstracta modelos LLM a través del `OpenCodeGoProvider` para generar narrativas que expliquen el puntaje asignado.

```mermaid
sequenceDiagram
    participant Client
    participant Controller as ScoringController
    participant Producer as CompanyScoringBatchProducer
    participant Queue as BullMQ (Redis)
    participant Worker as ScoringWorker
    participant LLM as LlmProvider (OpenCodeGo)
    participant DB as PostgreSQL (ScoreLog)

    Client->>Controller: POST /api/scoring/:companyId/batch
    Controller->>Producer: produceBatch(noticeUUIDs)
    Producer->>Queue: Enqueue "company-batch-scoring" job
    Controller-->>Client: 202 Accepted (jobId)

    Queue-->>Worker: Consume Job
    loop Para cada Notice UUID
        Worker->>Worker: Evaluar Hard Filters y Score de Afinidad
        Worker->>LLM: chat (generar justificación narrativa)
        alt LLM falla o time-out
            LLM--xWorker: Error
            Worker->>Worker: Fallback: Generar explicación basada en reglas
        else LLM responde con éxito
            LLM-->>Worker: Narrativa generada
        end
        Worker->>DB: Upsert MatchingResult
        Worker->>DB: Insert ScoreLog (Registro histórico de auditoría)
    end
```

### 5.2 Estructura del LLM Provider

Para prevenir el vendor lock-in y permitir fácil escalabilidad a otros LLMs, se diseñó la interfaz `LlmProvider`.
...
classDiagram
    ...
    OpenCodeGoProvider ..|> LlmProvider : Implements
    ScoringWorker --> LlmProvider : Injects
```
